...
class Louis_System_RecursiveMenu{
	...
	public function recursiveMenu($sourceArr, $parents = 0,$level = 1, &$newMenu){
	 ...
	 $newMenu .= '<li class="dropdown">';
	 $newMenu .= '<a class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false" href="/'. $value['link']  .'">'. $value['name'] .' <span class="caret"></span></a>';
		...
		$newMenu .= '<li><a href="/'. $value['link']  .'">'. $value['name'] .'</a></li>';
	...
	*/
	...
	public function isSub($parent){
		...
		$sql = 'SELECT id FROM menu_items WHERE isHome="Yes" and parent = '.$parent;
